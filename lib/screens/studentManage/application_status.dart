...
import '../../plugins/dalgeurak-meal-application/lib/pages/convenience_food_teacher/page.dart';
...
class ApplicationStatus extends GetWidget<MealController> {
                  ...
                  Positioned(
                    ...
                    child: GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: Icon(Icons.arrow_back_ios_rounded, size: 26),
                      ),
                    ),
                  ),
                    ...
                    if (snapshot.hasData) {
                      ...
                      for (String mealTypeStr in tabBarWeekDayMenuList) {
                        ...
                        if (mealType == MealType.breakfast) {
                          tabBarMenuList.add("샌드위치");
                        }

                        List<Widget> widgetList = [];
                        for (String menuName in tabBarMenuList) {
                          List<DalgeurakConvenienceFood> convenienceList = [];

                          try {
                            convenienceList = (responseData.where((element) =>
                            (element['name'] == menuName &&
                                element['time'] == mealType.convertEngStr &&
                                controller.isSameDate(element['duration']['start'], nowTime.subtract(Duration(days: nowTime.weekday - 1))))).toList())[0]['applications'];
                          } catch (e) {
                            print("Error: $e");
                          }
  ...
  getStudentListWidget(List<DalgeurakConvenienceFood> convenienceFoodList) {
    if (convenienceFoodList.isEmpty) {
      return Center(child: Text("신청자가 없습니다.", style: TextStyle(fontSize: 16)));
    }
